...
struct ProjectAgentContextService: Sendable {
    ...
    nonisolated func renderBlock(for project: ScarfProject) -> String {
        ...
        lines.append("")
        lines.append("- **Dashboard widgets.** `<project>/.scarf/dashboard.json` renders into Scarf's Projects tab via a typed widget vocabulary (`stat`, `progress`, `text`, `table`, `chart`, `list`, `cron_status`, `log_tail`, `markdown_file`, `status_grid`, `kanban_summary`, …). The full catalog + field schema lives in `~/.hermes/skills/scarf-template-author/SKILL.md` § Widget Catalog. The viewer auto-refreshes on file changes — no manual reload needed.")
