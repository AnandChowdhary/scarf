...
Clawdia never asks for your account password. It also never holds an Apple-side cloud token — there's no "sign in with Scarf" anywhere.
...
5. Test connection — Clawdia SSHes in, looks for the `hermes` binary, saves on success
...
- **Nickname (optional)** — display name in Clawdia. Defaults to `user@host`.
- **Hermes binary hint (optional)** — leave empty unless you know `hermes` is at an unusual path. Clawdia prepends `~/.local/bin`, `/opt/homebrew/bin`, and `/usr/local/bin` to PATH automatically — those three cover ~95% of pipx + Homebrew installs. If your `hermes` lives elsewhere (custom virtualenv, system-managed install dir like `~/.hermes/bin`), set the hint to its absolute path.

Tap **Next**.

### 2. Choose: generate or import

- **Generate a new key** — recommended for most users. Clawdia creates a fresh Ed25519 keypair on-device. This is the right choice unless you have a specific reason to reuse an existing key.
...
If generating: Clawdia runs `CitadelSSHService.generateEd25519Key()` (pure-Swift, no `ssh-keygen`). About a second.

If importing: paste the private-key PEM in the top box and the matching public-key line in the bottom box. Clawdia validates the pair before accepting.
...
- **Accessibility (opt-in, v2.5.1+):** `kSecAttrAccessibleAfterFirstUnlock` + `kSecAttrSynchronizable=true`. Toggle from System → Security → "Sync SSH key with iCloud Keychain". The Keychain entry now syncs across signed-in Apple devices, end-to-end encrypted by iCloud Keychain (with Advanced Data Protection enabled, the encryption keys never leave your devices). Adding a second device no longer requires generating a fresh key — install Clawdia, sign in to the same Apple ID with iCloud Keychain enabled, the same key shows up.
...
Clawdia displays the public-key line in a monospaced selectable box, with a Copy button.

Tap **Copy public key**. Now go to your Hermes host (in another shell) and append it:

```bash
cat >> ~/.ssh/authorized_keys <<'EOF'
ssh-ed25519 AAAAC3...the-line-Clawdia-showed-you... scarf-ios-<device-name>
EOF
...
Back in Clawdia, tap **I've added this key**.

### 5. Test connection

Clawdia opens an SSH session, runs a single probe (`echo $HOME && which hermes`), and if both succeed, saves the server. Expect a 2–4 second wait.
...
2. In Clawdia: **Servers → tap the server → Edit → Hermes binary hint** → paste the absolute path (e.g. `/opt/scarf-tools/bin/hermes`).
...
- Custom SSH port? Confirm in Clawdia's server-edit screen.

If `ssh user@host` from another device on the same network also fails, fix that first — Clawdia can't connect to a host you can't ssh to.

### "Authentication failed" / "Permission denied (publickey)"

The public key you copied into `authorized_keys` doesn't match what Clawdia is offering. Common causes:

- **You pasted the wrong line.** Tap **Show public key** again in Clawdia and re-copy. Compare line-for-line against what's in `~/.ssh/authorized_keys`.
...
Clawdia strict-checks SSH host keys. If the host's key changed (new install, MITM unlikely but possible), use **Servers → Forget this server** and re-onboard — Citadel will accept the new host key on first connect.

### Onboarding succeeds but Dashboard shows zero sessions

Clawdia downloads a snapshot of `~/.hermes/state.db` over SFTP. If your Hermes install hasn't yet written the DB (no sessions ever started), the snapshot is empty. Start a session via the Mac app or `hermes chat` first, then pull-to-refresh the Dashboard.
...
- **No analytics.** Clawdia doesn't transmit any data to any third party.
...
- [Clawdia](ScarfGo) — feature tour, FAQs, limitations.
