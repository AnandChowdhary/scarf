...
**Voice mode controls:** Rich Chat on macOS and Clawdia on iOS have a **Text / Voice** composer toggle. Voice mode streams microphone PCM into an OpenAI Realtime session (`gpt-realtime` with `gpt-realtime-whisper` input transcription), uses server VAD to submit after two seconds of post-speech silence, and sends the resulting text through the same Hermes ACP prompt path as typing. Hermes still owns the conversation, reasoning, tools, permissions, memory, and persisted transcript. As Hermes text streams, Clawdia sends only new stabilized text chunks through one persistent Realtime speech session, so playback can begin before the complete ACP turn settles without rereading the growing full reply. Clawdia's voice-first composer fills roughly the lower half of the chat. Its filled liquid-ribbon waveform is driven by a bounded RMS envelope derived from recent microphone or spoken-output PCM, while non-audio preparation/working states use only a restrained ambient pulse. It supports both tap-to-toggle and hold-to-talk, plays quiet synthesized waiting tones while Clawdia works, and automatically listens for a follow-up after speech playback. The ten-second follow-up timeout applies only when Realtime has not detected any speech; once speech begins, Realtime's silence event—not elapsed recording time—ends the utterance.

Voice is the continuous, background-capable experience; there is no separate Drive Mode. The first microphone press keeps one `.playAndRecord` / `.voiceChat` audio session configured across listening, Soft Lantern working audio, earcons, and streamed speech, and the ACP transport remains attached through screen lock. A tap starts hands-free server-VAD listening; holding starts push-to-talk and release submits immediately. Calls and Siri pause the conversation through audio-interruption notifications; a system resumption recommendation restarts listening, while the large microphone remains the manual resume control when automatic resumption is inappropriate. Bluetooth or wired-output loss pauses instead of unexpectedly moving the conversation to the iPhone speaker, and reconnecting a route resumes it. Media-services resets follow the same pause/rebuild path. Ten seconds without speech pauses listening; ten further minutes of inactivity ends the voice conversation, deactivates audio, and restores normal background suspension. A compact **End conversation** control is present while Voice owns background audio, and switching back to Text ends it too.

**Settings → Realtime Voice (Clawdia)** configures all ten built-in Realtime voices, output speed from 0.25× to 1.5×, delivery-style presets, and optional custom speaking guidance. Changes apply to the next spoken reply because a Realtime voice cannot change after its session begins emitting audio. The bring-your-own OpenAI key is stored device-locally in Keychain and can be replaced or forgotten from the composer. Terminal mode's existing Hermes PTT / TTS controls and Hermes provider preferences in **Settings → Voice** remain unchanged.
...
---
_Last updated: 2026-07-12 — unified background-capable Voice conversation_
## Siri, Shortcuts, Spotlight, and the Action button

Clawdia publishes four App Shortcuts as soon as the app is installed:

- **Talk to Clawdia** starts a fresh Clawdia session and begins a Voice conversation.
- **Continue Clawdia** resumes the most recently active Clawdia session.
- **Talk About Project** accepts a registered project name, opens a project-scoped chat, and begins a Voice conversation.
- **Capture Idea** asks for an idea and immediately sends it to a fresh Clawdia session.

The shortcuts can be invoked from Siri and Spotlight, composed into automations in the Shortcuts app, or assigned to the iPhone Action button. A cold-launch request is held for up to one hour while Clawdia connects. One-server setups connect automatically; when multiple servers are configured, Clawdia waits for the user to choose the intended server before completing the action.
